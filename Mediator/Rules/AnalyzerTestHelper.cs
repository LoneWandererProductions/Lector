 ...
 *
 * WHY THIS EXISTS:
 * ICodeAnalyzer.Analyze(filePath, fileContent) looks like it should work from pure
 * in-memory content, but CoreHelper.ShouldIgnoreFile (called first thing in every
 * analyzer) reads the file from disk via File.ReadLines(filePath) to check for
 * "<auto-generated>" headers - it does not use the fileContent argument for that
 * check. If filePath doesn't exist, that read throws, the catch treats the file as
 * "ignore", and Analyze silently returns zero diagnostics regardless of content.
 * So every test needs a real file on disk, not just a string.
