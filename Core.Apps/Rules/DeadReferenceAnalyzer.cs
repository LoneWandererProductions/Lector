    ...
    public sealed class DeadReferenceAnalyzer : ICommand, ICodeAnalyzer
    {
        ...
        public IEnumerable<Diagnostic> Analyze(string? filePath, string fileContent)
        {
            if (string.IsNullOrEmpty(filePath) ||
                !Path.GetExtension(filePath).Equals(".csproj", StringComparison.OrdinalIgnoreCase))
                yield break;
        ...
        public CommandResult Execute(params string?[] args)
        {
            ...
            return CommandResult.Ok(
                output.Length > 0 ? $"Unused references detected:\n{output}" : "No unused references detected.",
                EnumTypes.Wstring);
        }
    }
}
